...
cardsDeck.addEventListener('click', () => {
  ...
  const img = new Image();
  const imgUrl = `../assets/MythicCards/${currentColor}/${curCard}.png`
  img.src = `${imgUrl}`
  img.onload = () => {
    currentCard.style.backgroundImage = `url(${imgUrl})`
  };
...
console.log('Чтобы изменить Древнего, нажми на кнопку "Изменить Древнего" и выбери нужную карту.\n Чтобы замешать колоду, нажми кнопку "Замешать колоду!".\n Чтобы показать следующую карту, нажми на колоду(карта рубашкой вверх).\n Когда колода закончится, нажми кнопку "Замешать колоду!", чтобы замешать новую колоду с теми же параметрами, или измени Древнего или уровень сложности. Каждый раз при изменении параметров, необходимо замешивать новую колоду.\nЧтобы закрыть правила, нажми в любом месте.')